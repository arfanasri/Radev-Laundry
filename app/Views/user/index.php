...
<div class="card">
    <div class="card-header">
        <div class="row">
            ...
            <div class="col-md-2 d-grid gap-2 pb-2">
                <button type="button" class="btn btn-sm btn-block btn-primary" data-bs-toggle="modal"
                    data-bs-target="#modaltampil" onclick="tambahUser()">
                    Tambah
                </button>
            </div>
        </div>
    ...
    <div class="modal-dialog modal-dialog-centered">
        <div class="modal-content">
            <div class="modal-header">
                <h1 class="modal-title fs-5" id="modalCariLabel">Cari User</h1>
...
<script>
    ...
    async function tambahUser() {
        try {
            const response = await axios.post('<?= url_to("user.tambah") ?>');
    ...
    async function ubahUser(id) {
        try {
            const response = await axios.post('<?= site_url("user/ubah") ?>/' + id.toString());
            document.querySelector("#isimodal").innerHTML = response.data;
        } catch (error) {
            console.error(error);
        }
    ...
    async function createUser() {
        ...
        try {
            ...
            const response = await axios.post('<?= url_to("api/user") ?>', data);
            perubahanData();
            modal.toggle();
            Swal.fire({
                icon: 'success',
    ...
    async function updateUser(id) {
        ...
        try {
            const data = ambilDataForm();
            const response = await axios.patch('<?= url_to("api/user") ?>/' + id.toString(), data);
            perubahanData();
            modal.toggle();
            Swal.fire({
                icon: 'success',
    ...
    async function deleteUser(id) {
        try {
            const response = await axios.delete('<?= url_to("api/user") ?>/' + id.toString());
            perubahanData();
        } catch (error) {
            console.error(error);
        }
    ...
    function ambilDataForm() {
        ...
        nama_user = document.querySelector("#nama_user").value;
